import torch
from packages.pfc.utils import one_hot
import os 
import numpy as np  
    ...
    def em_update(self, _triggered=False):
        ...
        with torch.no_grad():
            if self._online_em_stepsize is None:
                self.params.data = self._stats_acc / (self._p_acc.unsqueeze(-1) + 1e-8)
            else:
                s = self._online_em_stepsize
                self.params.data = (1. - s) * self.params + s * (self._stats_acc / (self._p_acc.unsqueeze(-1) + 1e-8))
...
class NormalArray(ExponentialFamilyArray):
    ...
    def __init__(self, num_var, num_dims, array_shape, min_var=0.0001, max_var=10., use_em=True, **kwargs):
